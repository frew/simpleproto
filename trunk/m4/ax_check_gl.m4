...
AC_MSG_CHECKING(whether we're on OS X)
gl_no_x="${no_x}"
m4_define([AX_CHECK_OSX_PROGRAM],
          [AC_LANG_PROGRAM([[ ]],
                           [[return 0]])])
save_libs="$LIBS"
LIBS="-framework Cocoa"
AC_LINK_IFELSE([AX_CHECK_OSX_PROGRAM],[gl_no_x="yes"; AC_MSG_RESULT(yes)], [AC_MSG_RESULT(no)])
LIBS="$save_libs"

AS_IF([test "X$gl_no_x" != "Xyes"],
...
AS_IF([test "X$ax_cv_check_gl_libgl" = Xno -a "X$gl_no_x" = Xyes],
